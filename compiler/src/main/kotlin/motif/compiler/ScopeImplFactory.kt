...
private constructor(
  ...
  private inner class Factory(private val scope: Scope) {
    ...
    private fun perDependencyLockFields(cachingStrategy: motif.CachingStrategy, useSelectiveCaching: Boolean): PerDependencyLockFields? {
      // Generate per-dependency lock fields only for BASELINE_WITH_LOCK_SELECTABLE
      // SMART_CACHE uses null field initialization with synchronized(this), not per-dependency locks
